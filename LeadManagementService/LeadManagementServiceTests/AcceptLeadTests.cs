...
namespace LeadManagementServiceTests
{
    ...
    {
        [Fact]
        public async Task Accept_Lead_Should_Return_Ok()
        {
            ...
            var acceptResponse = await controller.AcceptLead(5577421);

            acceptResponse.Should().NotBeNull();
            acceptResponse.GetType().Should().Be(typeof(OkResult));
        }
        
        [Fact]
        public async Task Accept_Lead_Should_Remove_Lead_From_New_Leads()
        {
            ...
            var acceptResponse = await controller.AcceptLead(5577421);
            var newLeadsAfterAccept = await controller.GetNewLeads();
            ...
            newLeadsAfterAccept.Should().NotBeNull();
            newLeadsAfterAccept.GetType().Should().Be(typeof(OkObjectResult));
            JsonConvert.SerializeObject(((OkObjectResult) newLeadsAfterAccept).Value, new JsonSerializerSettings
            {
                ContractResolver = new DefaultContractResolver{ NamingStrategy = new CamelCaseNamingStrategy() },
